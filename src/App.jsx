...
function App() {
  ...
  const handleSelectVideo = (videos) => {
    const isExist = selectedVideos.find((item) => item.id == videos.id);
    let count = Number(videos.price);

    if (isExist) {
      alert("Already Selected");
    } else {
      selectedVideos.forEach((videos) => {
        count += Number(videos.price);
      });
  ...
  let credit = selectedVideos.reduce((total, course) => total + Number(course.credit), 0);
                ...
                allVideos.map(videos => (
                  <div className='card'>
                    <div className='card-img'>
                      <img className='photo' src={videos?.img} alt="" />
                    </div>
                    <div className="card-title">
                      ...
                      <div className='info'>
                        <div className='price'>
                          ...
                          <div><p className='taka'>Price:{videos?.price}</p></div>
                        </div>
                        <div>
                          ...
                          <div>
                            <p className='credite-name'><span>Credite:{videos.credit}</span>hr</p>
                          </div>
                        </div>
                      </div>
                      <div className="card-actions">
                        <button onClick={() => handleSelectVideo(videos)} className="card-btn">Select</button>
                      </div>
                    </div>
                  </div>
                ))}
